...
import { loadColumns } from './configureSubcategoriesCols';
...
const TableSampleSubcategories = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  useEffect(() => {
    if (subcategoriesNotify.showNotification) {
      notify(
        subcategoriesNotify.typeNotification,
        subcategoriesNotify.textNotification,
      );
    }
  }, [subcategoriesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/subcategories/subcategories-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {subcategories && Array.isArray(subcategories) && !showGrid && (
        <ListSubcategories
...
export default TableSampleSubcategories;
